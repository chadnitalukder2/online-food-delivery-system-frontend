...
const deleteDelivery = (id) => {
  axios.delete(`/api/deliveries/${id}`).then(() => {
    notify({
      title: "Delivery Item Deleted",
      type: "success",
    });
    getDelivery();
  });
  ...
  <div class="container">
    <div class="table-box">
      ...
      <table id="customers">
        ...
        <tbody v-for="item in deliveries" :key="item.id">
          <Modal :show="deleteVisibleId === item.id" @close="closeModalDelete">
            <div id="myModal" style="text-align: center;">
              ...
              <div class="modal_footer" style="padding: 20px;">
                ...
                <button @click="deleteDelivery(item.id)" type="button" style="background: #f15e5e;">Delete</button>
              </div>
            </div>
          </Modal>
          <tr>
            <td># {{ item.id }}</td>
            <td>{{ item.order_id  }}</td>
            <td>{{ item.delivery_personnel_id  }}</td>
